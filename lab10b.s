/*In this activity, I wrote a program in RISC-V assembly language that has a ABI-compliant recursive 
function int recursive_tree_search(Node *root, int val). My code was linked to a code in C that called 
the recursive_tree_search function and expected the return value to be the depth of the value if the 
searched value was present on the tree, 0 if not. Althought it would be trivial to write such a code
in C language, this lab was particularly very challenging because of the use of recursion, which
required me to be cautious with register ra and its changes. */
.bss
input: .skip 1
output: .skip 1
...
# a0: adress of root node, a1: value we wish to find
    addi sp, sp, -4
    sw ra, 0(sp)
    li a2, 0 # In case we do not find wished value on the binary tree
    li t0, 1 # Register t0 will be used as iterator to store current depth
    jal recursion
    # Bellow, we will deal with result
    mv a0, a2 
    lw ra, 0(sp)
    addi sp, sp, 4
    ret
    recursion:
        lw t1, 0(a0)
        beq t1, a1, found # Wished value is found on current node
        
        addi t0, t0, 1

        addi sp, sp, -4
        sw a0, 0(sp)
        lw a0, 4(a0) # We go to left child
        beqz a0, left_null # Left child is null and we skip it
        addi sp, sp, -4
        sw ra, 0(sp)
        jal recursion
        lw ra, 0(sp)
        ...
        left_null:
            ...
            addi sp, sp, -4
            sw a0, 0(sp)
            lw a0, 8(a0) # Go to right child
            beqz a0, right_null # Right child is null and we skip it
            addi sp, sp, -4
            sw ra, 0(sp)
            jal recursion
            lw ra, 0(sp)
...
a2: numeric base
    ...
    li t0, 10
    li t1, 0 # If positive or 0, output will have at least 1 digit
    beq t0, a2, decimal
    li t0, 16
