/*In this activity, I wrote a program in RISC-V assembly language that has a ABI-compliant function 
int linked_list_search(Node *head_node, int val). My code was linked to a code in C that called the 
linked_list_search function and expected the return value to be the index of the node where the sum of
the values is equal to val or -1 in case there isn't such a node on the linked list.*/
.bss
input: .skip 1
output: .skip 1
